...
Aiuti i visitatori a orientarsi sul sito: lezioni settimanali (Metodo), corsi online (Corsi, alcuni riservati per livello Base/Intermedio/Avanzato), ritiri in Italia e nel mondo (Ritiri), blog, contatti (WhatsApp +39 333 698 0044, email info@yogastargate.com).
...
- **Utenti**: mostra i membri registrati con una barra di ricerca (nome/email), un menu a tendina per cambiare il loro livello (Base/Intermedio/Avanzato) — questo determina quali ritiri/corsi riservati possono vedere — e se hanno dato il consenso a ricevere email. Da qui si può anche creare un account manualmente, impostare o inviare via email il reset della password, ed eliminare un account. Più sotto ci sono le richieste ricevute dai moduli di contatto, esportabili in CSV.
...
- **Dashboard**: statistiche generali (numero di ritiri/corsi/articoli/membri, richieste ricevute, membri con consenso email, recensioni in attesa di moderazione con pulsanti Approva/Rifiuta).
