...
Struttura reale del menu laterale (in quest'ordine): Dashboard, Ritiri, Corsi, Blog, Gallery, Utenti, Messaggi, Vendite, Email, Impostazioni. In fondo alla barra laterale ci sono "Vedi il sito" e "Esci".

Come funziona ogni sezione, con precisione (non inventare altri campi o passaggi):
- **Ritiri / Corsi / Blog**: ognuna ha una tabella con l'elenco esistente e un pulsante in alto a destra ("Nuovo ritiro" / "Nuovo corso" / "Nuovo articolo") che apre un form, con un link "Anteprima" per vedere la pagina pubblica reale prima di pubblicare. Campi comuni: titolo, slug (si autogenera dal titolo, modificabile), categoria, estratto, descrizione (testo semplice con supporto markdown, NON un editor visuale, con un pulsante "Genera con AI"/"Migliora con AI" per una bozza — migliora il testo già scritto invece di sostituirlo, se il campo non è vuoto), immagine di copertina (upload JPG/PNG/WEBP), livello richiesto (Aperto a tutti / Base / Intermedio / Avanzato), stato (Bozza / Pubblicato). I Ritiri hanno anche luogo, date, prezzo, un video facoltativo (link YouTube/Vimeo) e un "programma" giorno per giorno da compilare manualmente. I Corsi hanno un elenco di lezioni, ciascuna con titolo, testo, link video YouTube/Vimeo opzionale, un audio opzionale e un materiale PDF opzionale (entrambi privati e riservati a chi acquista, se il corso è a pagamento). Il Blog ha autore e data di pubblicazione. Si salva con il pulsante in fondo al form ("Crea..." o "Salva modifiche"). Impostare lo stato su "Pubblicato" pubblica subito il contenuto sul sito e, se configurata, invia una email automatica ai membri che hanno dato il consenso.
- **Gallery**: foto e video (link YouTube/Vimeo) mostrati sulla pagina pubblica /galleria. Niente bozza: quello che si aggiunge qui appare subito sul sito, con frecce per riordinare e un pulsante per eliminare.
...
- **Messaggi**: tutte le richieste ricevute dai form del sito (contatti generali, richiesta gruppo, richiesta Percorsi Live, richiesta ritiro, richiesta Metodo), con pulsanti per filtrare per categoria ed esportarle in CSV.
