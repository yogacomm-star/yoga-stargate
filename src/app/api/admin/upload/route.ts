...
const ALLOWED_TYPES: Record<string, { ext: string; folder: string; maxSize: number }> = {
  ...
  "audio/mpeg": { ext: "mp3", folder: "audio", maxSize: 150 * 1024 * 1024 },
};
...
export async function POST(request: Request) {
  ...
  if (!spec) {
    return NextResponse.json({ error: "Formato non supportato. Usa JPG, PNG, WEBP o MP3." }, { status: 400 });
  }
  ...
  await prisma.mediaAsset.create({
    data: { key, url, sizeBytes: file.size, mimeType: file.type },
  });
