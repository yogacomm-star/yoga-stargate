...
// Come groqChat, ma forza la risposta a essere un oggetto JSON valido (usato per
// generare bozze strutturate con più campi in un'unica chiamata).
export async function groqJson<T>(messages: ChatMessage[], maxTokens = 1200): Promise<T> {
  const key = process.env.GROQ_API_KEY;
  if (!key) throw new Error("GROQ_API_KEY non configurata.");
